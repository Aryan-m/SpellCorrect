...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in xrange(1, len(sentence) - 1):  # ignore <s> and </s>
      ...
      word = sentence[i]
      editList = self.editModel.editProbabilities(word)
      for editWord, editProb in editList:
        sentence[i] = editWord
        lmScore = self.languageModel.score(sentence)
        score = editProb + lmScore
        # check for best score
        if bestScore <= score:
          bestScore = score
          placement = i
          correctedWord = editWord

      sentence[i] = word
    bestSentence = sentence
    bestSentence[placement] = correctedWord
    return bestSentence
